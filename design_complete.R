...
n_iter <- 100 # Number of iterations of the simulation loop
...
for (i in 1:n_iter) {
  ...
  mu <- cbind(
    e * latent$activist_id, #env_norms_ingroup
    e * latent$activist_id + -e * design$tactic_radical + e * design$protest_size, #env_norms_outgroup
    -e * design$behav_implication + -e * latent$cause_affinity + -e * design$tactic_radical + e * design$protest_size, #efficacy_norms
    -2*e * latent$cause_affinity + e * design$tactic_radical, #opposition_norms
    e * design$behav_implication + e * latent$cause_affinity + e * design$tactic_radical, #dissonance
    -e * latent$cause_affinity + e * design$tactic_radical, #reactance
    -e * design$protest_size + e * design$tactic_radical #disidentification
  )
  ...
  residuals <- mvrnorm(n = N, mu = rep(0, ncol(mu)), Sigma = Sigma_resid)
  ...
  latent$dissonance = outcomes[,5]
  latent$reactance = outcomes[,6]
  latent$disidentification = outcomes[,7]
  ...
  di_items <- simulate_indicators(latent$dissonance, 3)
  re_items <- simulate_indicators(latent$reactance, 3)
  de_items <- simulate_indicators(latent$disidentification, 3)
  ...
  names(di_items) <- c("di1", "di2", "di3")
  names(re_items) <- c("re1", "re2", "re3")
  names(de_items) <- c("de1", "de2", "de3")
  ...
  dissonance    	   =~ di1 + di2 + di3
  reactance    	     =~ re1 + re2 + re3
  disidentification  =~ de1 + de2 + de3
  ...
  dissonance         ~ behav_implication + cause_affinity + tactic_radical
  reactance          ~ cause_affinity + tactic_radical
  disidentification  ~ protest_size + tactic_radical

  env_norms_ingroup  ~ activist_id
  env_norms_outgroup ~ disidentification + activist_id + tactic_radical + protest_size
  efficacy_norms ~ dissonance + cause_affinity + tactic_radical + behav_implication + protest_size
  opposition_norms   ~ reactance + cause_affinity + tactic_radical
...
layout_coords[match("behav_implication", nodes), ] <- c(1, 1)
layout_coords[match("tactic_radical", nodes), ] <- c(2, 1)
layout_coords[match("protest_size", nodes), ] <- c(3, 1)
layout_coords[match("activist_id", nodes), ] <- c(4, 1)
layout_coords[match("cause_affinity", nodes), ] <- c(5, 1)
# Moderators (middle, y=5), spaced horizontally (x=1 to 3)
layout_coords[match("dissonance", nodes), ] <- c(1.5, 5)
layout_coords[match("reactance", nodes), ] <- c(3, 5)
layout_coords[match("disidentification", nodes), ] <- c(4.5, 5)
# DVs (top, y=9), spaced horizontally (x=1 to 4)
layout_coords[match("env_norms_ingroup", nodes), ] <- c(1, 9)
layout_coords[match("env_norms_outgroup", nodes), ] <- c(2.5, 9)
layout_coords[match("efficacy_norms", nodes), ] <- c(3.5, 9)
layout_coords[match("opposition_norms", nodes), ] <- c(5, 9)
